...
<head>
	...
	<title>Détails du financement</title>
	...
	<div class="row ideaDetails">
		...
		<div class="container">
			<div class="page-header">
		 		<h3>Management des Funds</h3>
		 	</div>
			<c:choose>
					<c:when test="${not empty requestScope.funds}">
						<table id="manageFunds" class="table table-striped ">
							...
							<tbody>
								<c:forEach var="fund" items="${requestScope.funds}">
									<tr>
					                    <td>${fund.id}</td>
					                    <td>${fund.amount}</td>
					...
					<c:otherwise>
					<p>Il n'y a pas de fund associé</p>
					</c:otherwise> 
				</c:choose>
		</div>
</body>
